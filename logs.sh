...
if [ $userid -ne 0 ]
then 
     echo -e "$R error:: please run the script with root user $N" &>>$log_file
     exit 1
else
...
VALIDATE(){
    ...
    then
        ...
        echo -e "installing $2 is ... $R not success $N" &>>$log_file
    fi
}
...
then 
    ...
    echo -e "$Y mysql already installed...nothing to do $N" &>>$log_file
fi
